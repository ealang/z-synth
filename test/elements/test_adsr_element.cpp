...
void expect_buffer_eq(std::vector<float> expected, ADSRElement& elem) {
  ...
  for (uint32_t i = 0; i < num; ++i) {
    ...
    EXPECT_TRUE(abs(expected_sample - actual_sample) < 0.001) << "Index " << i << ": Expected " << expected_sample << ", got " << actual_sample;
  }
}
